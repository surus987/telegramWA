...
var Cafe = {
  ...
  init: function (options) {
    ...
    Cafe.currency = options.currency;
    ...
    if (!Telegram.WebApp.initDataUnsafe ||
      ...
      let tgrm = window.Telegram.WebApp; //получаем объект webapp телеграма 
      tgrm.expand(); //расширяем на все окно 
      var first_name = Telegram.WebApp.initDataUnsafe.user.first_name;
      window.alert(`${first_name},\n\nby clicking on the name of the product next to the price, an enlarged image and a detailed description of the product will open!`);
    }
  ...
  formatPrice: function (price) {
    return Cafe.currency + Cafe.formatNumber(price / 1000, 2, '.', ',');
  },
    ...
    if (Cafe.modeOrder) {
      if (Cafe.isLoading) {
        ...
        mainButton.setParams({
          ...
          text: 'CONFIRM ' + Cafe.formatPrice(Cafe.totalPrice),
          color: '#31b545'
        }).hideProgress();
      }
    } else {
  ...
  getOrderData: function () {
    ...
    if (Telegram.WebApp.initData) {
      order_data.push({ auth: Telegram.WebApp.initData })
    } else order_data.push({ auth: "null" });
    var comment = $('.js-order-comment-field').val();
    order_data.push({ comment: comment });
    return order_data;
  },
  ...
  mainBtnClicked: function () {
    ...
    if (Cafe.modeOrder) {
      ...
      Cafe.apiRequest(function (result) {
  ...
  apiRequest: function (onCallback) {
    $.ajax(Cafe.apiUrl, {
      method: 'POST',
      //data: { webapp: Telegram.WebApp.initData},
      data: { webapp: Cafe.getOrderData() },
